...
function sendToOBD2(data) {
    if(data.indexOf("01") == 0) {
        if(capabilities.includes(data.substr(0,2))){
            sendBuffer.push(data);
        } else {
            console.log("sendToOBD2: Error request "+data+" not supported");
        }
    } else {
        sendBuffer.push(data);
    }
            ...
            if (pid.realtime) {
                ...
                checkBoxNode.addEventListener('change', (event) => {
                    if(event.target.checked) {
                        console.log("checked box: ",checkBoxNode);
                    }
                });
    ...
    win.socket.on('data', function (data) {
        ...
        if (commandArray.length <= 1) {
            ...
            for (var i = 0; i < commandArray.length; i++) {
                ...
                var lines = command.split("\r").filter(Boolean);
                console.log(lines);
